# ProITFix-FieldToolkit_v6_FINAL.ps1
...
$global:reportFile = "$tempPath\FTK_Report_$timestamp.txt"
$global:summaryFile = "$tempPath\FTK_Summary_$timestamp.txt"
...
function Run-Diagnostics {
    ...
    Write-Color "Diagnostics complete. Report saved to C:\Temp" Green
}
...
function Smart-Recommendation {
    ...
    if (-not $analysis) {
        Write-Color "⚠️ Run Diagnostics first (Option 1)" Red
        return
    }
...
"@
    ...
    Write-Color "`nSummary saved to C:\Temp and copied to clipboard." Green
}

# =============================
# PRINTER TOOLS (🔥 RESTORED)
# =============================
function Printer-Tools {

    if (-not (Confirm-Action "Run Printer Diagnostics?")) { return }

    Write-Color "`n==== PRINTER DIAGNOSTICS ====" Cyan

    $printers = Get-Printer

    if ($printers) {
        $printers | Format-Table Name, Default, Status

        $default = $printers | Where-Object {$_.Default -eq $true}

        if (-not $default) {
            Write-Color "`n⚠️ No default printer set (Epic risk)" Red
        } else {
            Write-Color "`nDefault Printer: $($default.Name)" Green
        }
    } else {
        Write-Color "No printers found" Red
    }
}
...
function Run-All {
    ...
    if (-not (Confirm-Action "Run FULL AUTO MODE?")) { return }
